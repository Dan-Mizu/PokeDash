...
<template>
	...
	>
		<Spinner v-if="!store.pokemonSprites[pkmnRef(data)]" />
		<img
			v-else
			class="w-fit"
			:src="store.pokemonSprites[pkmnRef(data)]"
	...
	>
		...
		<!-- Show More Button -->
		<button @click="showExtraInfo = !showExtraInfo" class="cursor-pointer">
			<Icon
				v-if="showExtraInfo"
			...
			<div class="mb-2">
				...
				<span class="text-base flex items-center mb-1">
					<Icon
						...
						class="h-4 text-light-tertiary dark:text-dark-tertiary"
					/>
					<span class="text-xs ml-1"> {{ data.ability }}</span></span
				>
